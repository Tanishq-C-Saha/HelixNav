...
from isaaclab.sensors import MultiMeshRayCasterCfg, MultiMeshRayCaster, patterns
...
def design_scene():
    ...
    for i, pos in enumerate(obs_positions):
        ...
        cubes.append(RigidObject(cfg=cube_cfg))    
    ...
    # Apply physics schemas that the USD doesn't have
    stage = sim_utils.get_current_stage()
    prim = stage.GetPrimAtPath("/World/Cardboard")

    # Apply RigidBodyAPI — makes it a dynamic body
    UsdPhysics.RigidBodyAPI.Apply(prim)

    # Apply CollisionAPI — makes PhysX detect contacts
    ...
    sensor_base_cfg = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=(0.0, 0.0, 1.0), rot=(0.7071068, 0, -0.7071068, 0)
        ),
    )
    ...
    sensor = MultiMeshRayCaster(cfg=sensor_cfg)
...
def run_simulation(sim, cubes, sensor, sensor_base, keyboard):
        ...
        with torch.inference_mode():
            ...
            if count % 50 == 0:
                ...
                world_dir = math_utils.quat_apply(
                    sensor_quat.unsqueeze(0), local_dir.unsqueeze(0)
                )[0]
            ...
            if count % 10 == 0:
                print(
                    f"[DEBUG] commanded wz: {wz:.3f} | actual wz after step: {sensor_base.data.root_com_vel_w[0, 5].item():.3f}"
                )
...
def main():
    ...
    # Check 1: kinematic flag
  
    stage = sim_utils.get_current_stage()
    prim = stage.GetPrimAtPath("/World/Cardboard")
    rigid_api = UsdPhysics.RigidBodyAPI(prim)
    print(f"[DEBUG] kinematic: {rigid_api.GetKinematicEnabledAttr().Get()}")

    # Check 2: instanceable
    print(f"[DEBUG] instanceable: {prim.IsInstanceable()}")
